...
- 작업 그룹 생성
  - 작업 그룹 이름 설정
  - Performance : 기본 용량 → RPU `8` 설정
- 네임스페이스 생성
  - 네임스페이스 이름 설정
  - 관리자 사용자 이름 / 비밀번호 설정
  - IAM Role 연결 (`skills-redshift-role`)
---
### Serverless Connect
- 쿼리 편집기 v2
- 작업 그룹 클릭 → 사용자 이름 / 비밀번호로 연결
---
...
3. S3에 Query 결과 올리기
```sql
UNLOAD ('SELECT * FROM employee')	# 이 UNLOAD 안에서 실행하는 query의 결과를 저장하겠다.
TO 's3://{버킷명}/output/'	# 결과 올릴 s3 folder 경로
IAM_ROLE '{role arn}'
FORMAT AS CSV
HEADER;
```
...
DATABASE '{glue에 생성할 db명}'	# Glue 에 이런 이름의 DataBase를 생성하겠다.
IAM_ROLE '{role arn}'	# Redshift에 붙어 있는 role arn
...
STORED AS TEXTFILE
LOCATION 's3://{버킷명}/{폴더}/'	# LOCATION은 항상 폴더까지만
